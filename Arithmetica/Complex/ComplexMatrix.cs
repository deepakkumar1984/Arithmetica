    ...
    public class ComplexMatrix : IEnumerable<Complex>
    {
        ...
        public Complex Max
        {
            get
            {
                ...
                {
                    for (int j = 0; j < ColumnCount; j++)
                        max = Complex.Maximum(max, _m[i, j]);
                }
        ...
        public static ComplexMatrix[] LUPDecomposition(ComplexMatrix m)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    if (Complex.Abs(c[row, i]).X > pivotValue)
                    {
                        pivotValue = Complex.Abs(c[row, i]).X;
                        pivot = row;
                    }
                }
                if (pivotValue == 0)
        ...
        public static int Rank(ComplexMatrix m)
        {
            ...
            while (i < m.RowCount && j < m.ColumnCount)
            {
                ...
                {
                    if (Complex.Abs(m[k, j]).X > pivotValue)
                    {
                        pivot = k;
                        pivotValue = Complex.Abs(m[k, j]).X;
                    }
                }
        ...
        public static double InfinityNorm(ComplexMatrix m)
        {
            ...
            if (m.IsVector)
            {
                ...
                for (int i = 0; i < m.Length; i++)
                {
                    if (max < Complex.Abs(m[i]).X)
                        max = Complex.Abs(m[i]).X;
                }
        ...
        public static float FrobeniusNorm(ComplexMatrix m)
        {
            ...
            for (int i = 0; i < m.RowCount; i++)
            {
                for (int j = 0; j < m.ColumnCount; j++)
                {
                    norm += Math.Pow(Complex.Abs(m[i, j]).X, 2);
                }
            }
        ...
        /// </returns>
        public static double PNorm(ComplexMatrix m, double p)
        {
            if (p <= 0)
            ...
            for (int i = 0; i < m.RowCount; i++)
            {
                for (int j = 0; j < m.ColumnCount; j++)
                {
                    norm += Math.Pow(Complex.Abs(m[i, j]).X, p);
                }
            }
        ...
        public float AbsColumnSum(int index)
        {
            ...
            for (int i = 0; i < RowCount; i++)
                result += Complex.Abs(this[i, index]).X;
        ...
        public float AbsRowSum(int index)
        {
            ...
            for (int j = 0; j < ColumnCount; j++)
            {
                result += Complex.Abs(this[index, j]).X;
            }
