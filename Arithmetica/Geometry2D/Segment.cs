...
namespace Arithmetica.Geometry2D
{
	...
	public struct Segment : ICloneable
	{
		...
		/// </summary>
		/// <param name="p0">A <see cref="Vector2"/> instance marking the segment's starting point.</param>
		/// <param name="p1">A <see cref="Vector2"/> instance marking the segment's ending point.</param>
		...
		public static Segment Parse(string value)
		{
			...
			{
				return new Segment(
					Vector2.Parse(m.Result("${p0}")),
					Vector2.Parse(m.Result("${p1}"))
					);
			}
			else
			{
