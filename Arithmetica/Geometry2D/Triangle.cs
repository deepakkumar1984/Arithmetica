...
namespace Arithmetica.Geometry2D
{
	...
	public struct Triangle : ICloneable
	{
		...
		/// <param name="p2">A <see cref="Vector2"/> instance.</param>
		...
		public static Triangle Parse(string value)
		{
			...
			if (m.Success)
			{
				return new Triangle(
					...
					Vector2.Parse(m.Result("${p2}")),
					Vector2.Parse(m.Result("${p3}"))
					);
			}
			else
			{
		...
		/// </summary>
		/// <value>A <see cref="Vector2"/> instance.</value>
		...
		public Vector2 FromBarycentric(float u, float v)
		{
